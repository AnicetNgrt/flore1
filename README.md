...
```
I haven't really explained `Refresh.feed()`'s syntax already, so let me do it. The first argument must be a function or a method (ex: `my_function`, or `Object.method`), the second argument must be a tuple of keyword arguments for the function (ex: `(x,y,0,18,"ok")`), and the third one must be a di
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcyNTY1OTgyOCwtMTMxNzgxNjE4OF19
-->